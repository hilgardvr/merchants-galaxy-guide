...
class CommodityServiceSpec extends AnyFlatSpec with Matchers {
  "CommodityService" should "set and get commodity prices" in {
    ...
    CommodityService.getCommodityPrice("Gold") shouldEqual Some(10.2345)
    CommodityService.setCommodityPrice("Silver", 55)
    CommodityService.getCommodityPrice("Silver") shouldEqual Some(55)
    CommodityService.getCommodityPrice("Unknown") shouldEqual None
  }
}
